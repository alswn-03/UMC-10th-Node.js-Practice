...
export const getAllMyReviews = async (
  ...
  const reviews = await prisma.review.findMany({
    ...
    where: { 
      userId: BigInt(userId), // Prisma의 BigInt 타입과 맞추기 위해 BigInt로 변환
...
// const conn = await pool.getConnection();
  ...
  try {
    ...
    throw new Error(`가게를 찾는 중 오류가 발생했어요: ${err}`);
  }
};
...
export const addReview = async (
  ...
  try {
    ...
    return createdReview.id; // 이건 어떤... 뭘 반환하는 거지? 이 부분 문법이해가 안된다
  } catch(err) {
    throw new Error('리뷰를 작성하는 중 오류가 발생했어요: ${err}')
  }
};
